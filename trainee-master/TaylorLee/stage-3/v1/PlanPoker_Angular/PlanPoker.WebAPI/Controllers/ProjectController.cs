...
namespace PlanPoker.WebAPI.Controllers
{
    ...
    public class ProjectController : ApiController
    {
        ...
        [Route("api/project")]
        [HttpPost]
        public Project Create(ProjectViewModel projectViewModel)
        {
            projectViewModel.ProjectGuid = Guid.NewGuid().ToString();
            var project = projectViewModel.ConvertToProject();
            
            return _projectLogic.Create(project);
        }
        ...
        {
            var projects = _projectLogic.GetAll();
            var projectViewModels = new List<ProjectViewModel>();
            foreach (var project in projects)
            {
                var dateTime = project.StartDate.ToUniversalTime();
                projectViewModels.Add(project.ConvertToProjectViewModel());
            }
        ...
        {
            return _projectLogic.GetAll().Where(x => x.ProjectGuid == id).FirstOrDefault().Id.ToString();
        }
    }
}
