    ...
    public class UserLogic : IUserLogic
    {
        ...
        public string Create(UserLogicModel model)
        {
            ...
            if(!CheckIfExist(model.UserName))
            {
                ...
                {
                    _userRepository.Create(model.ConvertToModel());
                    unitOfwork.Commit();
                }
            }
            else
        ...
        public void Edit(UserLogicModel model)
        {
            using (var unitOfwork = _unitOfWorkFactory.GetCurrentUnitOfWork())
            {
                _userRepository.Edit(model.ConvertToModel());
                unitOfwork.Commit();
            }
        }
        ...
        {
            var users = _userRepository.GetByIds(x => ids.Contains(x.UserId));
            var userLogicModels = new List<UserLogicModel>();
            if (users == null) return userLogicModels;
