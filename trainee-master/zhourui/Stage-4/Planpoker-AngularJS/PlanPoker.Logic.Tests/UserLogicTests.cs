...
namespace PlanPoker.Logic.Tests
{
    ...
    public class UserLogicTests
    {
        ...
        public void Create_when_username_exist_should_return_specific_message()
        {
            ...
            userRepository.Setup(x => x.GetByName(It.IsAny<string>())).Returns(user);
            // Act
            var result = _userLogic.Create(user.ConvertToUserLogicModel("",false));
            // Assert
        ...
        public void Create_when_email_exist_should_return_specific_message()
        {
            ...
            var userRepository = _mocker.GetMock<IUserRepository>();
            userRepository.Setup(x => x.LoadAll()).Returns(users.ToList());
            // Act
            var result = _userLogic.Create(user.ConvertToUserLogicModel("",false));
            // Assert
        ...
        public void Create_when_username_and_email_not_exist_should_execute_once()
        {
            ...
            var userRepository = _mocker.GetMock<IUserRepository>();
            //userRepository.Setup(x => x.LoadAll()).Returns(users);
            userRepository.Setup(x => x.Create(user)).Returns(user);
            userRepository.Setup(x => x.Get(user.UserId)).Returns(user);
            // Act
            var result = _userLogic.Create(user.ConvertToUserLogicModel("", true));
            // Assert
        ...
        public void Edit_when_email_exist_and_email_not_equals_original_email_should_return_specific_message()
        {
            ...
            var userRepository = _mocker.GetMock<IUserRepository>();
            userRepository.Setup(x => x.LoadAll()).Returns(users.ToList());
            userRepository.Setup(x => x.GetByName("zhourui")).Returns(originalUser1);
            // Act
            var result = _userLogic.Edit(updatedUser.ConvertToUserLogicModel("",false));
            // Assert
        ...
        [Test]
        public void Edit_when_email_not_exist_should_execute_once()
        {
            // Arrange
            var user = new User
            {
            ...
            var userRepository = _mocker.GetMock<IUserRepository>();
            userRepository.Setup(x => x.LoadAll()).Returns(users);
            userRepository.Setup(x => x.GetByName(user.UserName)).Returns(user);
            // Act
            var result = _userLogic.Edit(user.ConvertToUserLogicModel("",true));
            // Assert
            userRepository.Verify(x => x.Edit(user), Times.Once());
            unitOfWork.Verify(x => x.Commit(), Times.Once());
            Assert.AreEqual(typeof(UserLogicModel), result.GetType());
            result.Password.Should().Be("321");
        }
        ...
        public void GetAll_should_execute_once_and_return_1()
        {
            ...
            // Assert
            userRepository.Verify(x => x.LoadAll(), Times.Once());
        ...
        public void Get_when_username_not_empty_should_return_right_type()
        {
            ...
            // Assert
            userRepository.Verify(x => x.GetByName(It.IsAny<string>()), Times.Once());
            Assert.AreEqual(typeof(UserLogicModel), result.GetType());
            result.UserId.Should().Be(1);
            result.Message.Should().Be("");
        }
