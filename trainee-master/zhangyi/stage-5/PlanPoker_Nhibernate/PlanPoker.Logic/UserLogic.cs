...
namespace PlanPoker.Logic
{
    public class UserLogic : IUserLogic
    {
        ...
        public void Edit(UserLogicModel model)
        {
            using (var unitOfwork = _unitOfWorkFactory.GetCurrentUnitOfWork())
            {
                var modelInSession = _userRepository.Get(model.UserId);
                modelInSession.Email = model.Email;
                modelInSession.Image = model.Email;
                modelInSession.Password = model.Password;
                _userRepository.Edit(modelInSession);
                unitOfwork.Commit();
            }
        }
