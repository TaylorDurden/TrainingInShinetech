...
angular.module("mainApp").controller('estimateController', ['$scope', '$http', function ($scope, $http) {
    $scope.projectName = '';
    ...
    $scope.onProjectEstimateInit = function () {
        $http({
            ...
            refresult = setInterval(function () { $scope.queryAppendPoker() }, 500);
        }).error(function (e) {
        });
    };
    ...
    $scope.showPokerResult = function () {
        ...
        changeIsShow($http);
        ref = setInterval(function () { $scope.queryFinished() }, 500);
    };
